...
data = cv([1, 2])
features_counts = []
for order in [1, 10, 20, 30, 40, 50]:
    phi = make_polynomial_feature_fun(order)
    transformed = phi(data)
    features_count = np.shape(transformed)[0]
    print("order = {}, feature_count= {}".format(order, features_count))
    features_counts.append(features_count)
